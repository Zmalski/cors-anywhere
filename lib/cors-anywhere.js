...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    if (location.host === 'logs') {
      // Check if logs file exists
      if (!fs.existsSync(corsAnywhere.logsFile)) {
        res.writeHead(404, 'Not found', cors_headers);
        res.end('Logs file not found');
      } else {
        // Show logs file
        res.writeHead(200, 'OK', cors_headers);
        fs.createReadStream(corsAnywhere.logsFile).pipe(res);
        // Check logs file size and clear if above 1MB
        // Not an ideal way to do this, but it's good enough for now
        fs.stat(corsAnywhere.logsFile, function(err, stats) {
          if (err) {
            console.error(err);
          } else if (stats.size > 1000000) {
            fs.writeFile(corsAnywhere.logsFile, '', function(err) {
              if (err) {
                console.error(err);
              }
            });
